...
*   **Authentication & Session Persistence:** SMS OTP verification with JWT Bearer tokens and 30-day rotating refresh tokens (`/api/v1/auth/refresh`). The mobile client features automated background token renewal via Dio interceptors, offline-resilient startup state caching, and EncryptedSharedPreferences (Android Keystore / iOS Keychain) session storage.
