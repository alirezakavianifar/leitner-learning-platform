...
*   **[scripts/IOS_BUILD_GUIDE.md](file:///e:/projects/leitner-learning-platform/scripts/IOS_BUILD_GUIDE.md)**: Step-by-step guide for building, cloud CI pipelines, sideloading, and distributing the iOS version.
...
*   **Flashcard Layout:** Fixed header (displays course title, stage indicator, card number, and favorite toggle), rotating center card (with flip animation, support for images, audio, and multiple-choice options), and fixed footer (with navigation arrows, Know/Don't Know actions, and content report modal).
    *   *Multiple-Choice Questions (MCQ):* Cards support 4-option multiple-choice questions authored via dedicated columns (`firstoption` / `first option` ... `fourthoption` / `fourth option`, `option1` .. `option4`), comma/semicolon/newline-delimited lists, or JSON lists (`options`). Column names are automatically normalized across whitespace and punctuation, and literal `'NULL'` values are sanitized. Options render on the front as interactive selectable tiles and reset cleanly when switching cards.
    *   *BiDi Text Directionality:* Question, answer, and choice text dynamically resolve directionality based on content (`detectTextDirection`), preventing punctuation transposition (e.g. English terminal periods appearing on the visual left in Persian RTL mode).
...
*   **Profile Management & Cloud Avatars:** User can upload and customize their profile avatar (PNG, JPG, WEBP) which is automatically uploaded to the server (`POST /api/v1/user/avatar`) and persists across devices, logouts, session refreshes, and app reinstallations with resilient offline caching and graceful fallback. User can edit profile metadata (username, interests, educational field, educational level) while the mobile number field remains strictly read-only.
