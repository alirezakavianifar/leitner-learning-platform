...
*   **Flashcard Layout:** Fixed header (displays course title, stage indicator, card number, and favorite toggle), rotating center card (with flip animation, support for images, audio, and multiple-choice options), and fixed footer (with navigation arrows, Know/Don't Know actions, and content report modal).
    *   *Multiple-Choice Questions (MCQ):* Cards support 4-option multiple-choice questions authored via dedicated columns (`first option` ... `fourth option`) or JSON lists (`options`). Options render on the front as interactive selectable tiles and reset cleanly when switching cards.
