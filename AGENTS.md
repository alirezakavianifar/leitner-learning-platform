...
- **iOS Build Pipeline**: All iOS builds are built using GitHub Actions (`.github/workflows/build-ios.yml`) on `macos-14` cloud runners. The pipeline builds `app-premium-release.ipa` and packages it into `app-premium-ios-release.zip`, retaining all release packages securely as GitHub Actions artifacts for download.
